...
namespace lfs::training {
    ...
    KGS::KGS(lfs::core::SplatData& splat_data) : _splat_data(&splat_data) {}

    void KGS::initialize(const lfs::core::param::OptimizationParameters& optimParams) {
        ...
        if (_params->max_cap > 0) {
            ...
            LOG_INFO("KGS: pre-allocating capacity for {} Gaussians (current: {}, utilization: {:.1f}%)",
            ...
            auto ensure_shN_capacity_direct = [capacity, layout_rest, &is_interop_external,
                ...
                if (param.dtype() != DataType::Float32) {
                    LOG_DEBUG("KGS: skip pre-alloc grow of non-float shN (dtype={})",
    ...
    void KGS::set_training_dataset(std::shared_ptr<CameraDataset> views) {
    ...
    lfs::core::Tensor KGS::edge_score_scratch(const int iter) {
        using namespace lfs::core;
    ...
    void KGS::on_edge_score_accumulated(const int iter) {
    ...
    void KGS::pre_step(int iter, RenderOutput& render_output) {
        (void)render_output;
        publish_vram_attribution();
    ...
    size_t KGS::densification_row_count() const {
        return 2;
    }

    void KGS::ensure_densification_info_shape() {
        const size_t n = static_cast<size_t>(_splat_data->size());
    ...
    int KGS::view_target_samples_per_refine_window() const {
    ...
    bool KGS::should_accumulate_view_sample(int iter) const {
        if (!_params ||
            iter <= static_cast<int>(_params->start_refine) ||
            iter >= static_cast<int>(_params->stop_refine) ||
            _splat_data->size() == 0) {
    ...
    bool KGS::should_cache_seed_view(int iter) const {
    ...
    void KGS::cache_seed_view(int iter, const RenderOutput& render_output) {
        using namespace lfs::core;
        if (!should_cache_seed_view(iter) || !render_output.camera) {
            return;
        }
        LOG_TIMER("KGS::cache_seed_view");
    ...
    void KGS::accumulate_explore_sample(int iter, const RenderOutput& render_output) {
        using namespace lfs::core;
        ...
        LOG_TIMER("KGS::accumulate_explore_sample");
        if (!is_cuda_image(render_output.image) ||
            !is_cuda_image(render_output.target_image) ||
    ...
    void KGS::post_render(int iter, RenderOutput& render_output) {
        accumulate_explore_sample(iter, render_output);
        cache_seed_view(iter, render_output);
    }

    void KGS::post_backward(int iter, RenderOutput& render_output) {
        LOG_TIMER("KGS::post_backward");
        using namespace lfs::core;
    ...
    void KGS::permute_gaussian_rows(const lfs::core::Tensor& perm) {
    ...
    bool KGS::is_refining(int iter) const {
    ...
    int KGS::effective_grow_until_iter() const {
    ...
    void KGS::refine(int iter, RenderOutput& render_output) {
        lfs::core::alloc_counter::ScopedSite densify_site("densify");
        LOG_TIMER("KGS::refine");
        LFS_VRAM_SCOPE("KGS::refine");
        using namespace lfs::core;
        ...
        if (pruned_count > 0) {
            ...
            LOG_DEBUG("KGS: soft-pruned {} splats at iter {} (active: {}, total slots: {})",
    ...
    void KGS::refresh_camera_hull() {
        ...
        if (n_cam < 2) {
            ...
            if (!_logged_degenerate_hull && background_improvements_enabled()) {
                LOG_INFO("KGS: camera hull unavailable (need >= 2 training cameras); far-field guard is inert");
        ...
        if (counted < 2) {
            update_far_starvation();
            if (!_logged_degenerate_hull && background_improvements_enabled()) {
                LOG_INFO("KGS: camera hull unavailable (need >= 2 valid cameras); far-field guard is inert");
                _logged_degenerate_hull = true;
            }
            return;
        }
        ...
        if (!std::isfinite(radius) || radius <= radius_eps) {
            update_far_starvation();
            if (!_logged_degenerate_hull && background_improvements_enabled()) {
                LOG_INFO("KGS: camera hull degenerate (orbit radius {:.6g}); far-field guard is inert",
                         radius);
                _logged_degenerate_hull = true;
            }
            return;
        ...
        if (n_now > 0) {
            if (far_scene_min_fraction <= 0.0f) {
                ...
                LOG_INFO("KGS: deep-far census unconditional at threshold 0 (n={}, far features {})",
                ...
                LOG_INFO("KGS: deep-far fraction {:.4f} at {}x orbit (threshold {:.2f}, far features {})",
    ...
    int KGS::cadence_scaled(const int count) const {
    ...
    int KGS::starved_cadence_count(const int count) const {
    ...
    float KGS::effective_far_growth_cap() const {
        if (!background_improvements_enabled()) {
            return 1.0f;
        }
    ...
    float KGS::effective_far_decay_scale() const {
        if (!background_improvements_enabled()) {
            return 1.0f;
        }
    ...
    float KGS::effective_mean_step_ratio_max() const {
        if (!background_improvements_enabled()) {
            return 1.0f;
        }
    ...
    float KGS::far_starvation_factor(const float ratio, const float full, const float rich) {
    ...
    float KGS::explore_starvation_multiplier(const float vis_i, const float median_vis) {
        if (vis_i == 0.0f) {
            return 0.0f;
        }
    ...
    void KGS::update_far_starvation() {
        ...
        if (std::abs(s - _logged_far_starvation) > 0.1f) {
            if (census_forces_full) {
                LOG_INFO("KGS: far starvation 1.00 (far-field scene)");
            } else if (uncapped) {
                LOG_INFO("KGS: far starvation 0.00 (uncapped)");
            } else {
                LOG_INFO("KGS: far starvation {:.2f} (cap/points {:.2f})", s, ratio);
            }
    ...
    void KGS::refresh_far_field_mask(const size_t n) {
        using namespace lfs::core;
    ...
    void KGS::publish_mean_step_far_mask() {
        if (!_optimizer) {
            return;
        }
        if (!background_improvements_enabled()) {
    ...
    void KGS::begin_far_growth_window(const size_t n, const int reserved_seeds) {
        using namespace lfs::core;
    ...
    lfs::core::Tensor KGS::sample_gumbel_with_far_guard(
    ...
    void KGS::apply_explore_starvation_weights(lfs::core::Tensor& weights, const size_t n) {
        using namespace lfs::core;
        if (_topology_frozen) {
            return;
        }
    ...
    lfs::core::Tensor KGS::build_explore_split_weights(
        const size_t n,
    ...
    void KGS::grow_and_split(int iter, int pruned_count) {
        LOG_TIMER("KGS::grow_and_split");
        LFS_VRAM_SCOPE("KGS::grow_and_split");
        using namespace lfs::core;
        const size_t n = static_cast<size_t>(_splat_data->size());
        ...
        LOG_DEBUG("KGS: split {} splats at iter {} (reused: {}, appended: {}, active: {}, total slots: {})",
    ...
    lfs::core::Tensor KGS::build_replace_parent_weights(
        const size_t n,
        const lfs::core::Tensor& active_mask,
        const lfs::core::Tensor& trainable_mask,
    ...
    void KGS::compact_splats(const lfs::core::Tensor& keep_mask) {
        ...
        auto compact_shN_swizzled = [&](Tensor& t, size_t cap_rows, int uint8_fill = -1) {
            ...
            if (t.dtype() == DataType::Float32) {
                ...
                if (uint8_fill >= 0 && cap_floats > 0) {
                    ...
                    if (err != cudaSuccess) {
                        throw std::runtime_error(
                            std::string("KGS::compact_splats: cudaMemsetAsync failed: ") +
                ...
                throw std::runtime_error("KGS::compact_splats: unsupported swizzled shN dtype");
            }
            t = std::move(fresh);
        };
        ...
        for (auto pt : ALL_PARAMS) {
            ...
            LFS_DEBUG_ASSERT_MSG(state->capacity >= state->size,
                                 "KGS::compact_splats: state.capacity < state.size");
        }
        ...
        if (_splat_data->has_deleted_mask()) {
            if (_splat_data->deleted().numel() == old_size) {
                ...
                LOG_WARN("KGS::compact_splats: deleted mask numel {} != old size {}; reconciling",
    ...
    void KGS::inject_noise(int iter) {
        const size_t n = static_cast<size_t>(_splat_data->size());
        if (n == 0)
            return;
    ...
    void KGS::enforce_max_cap(int iter) {
        if (_params->max_cap <= 0)
            return;
        ...
        LOG_INFO("KGS: count {} exceeds max_cap {}, pruning excess", n, cap);
        ...
        if (frozen_mask.is_valid()) {
            ...
            if (frozen_count > cap) {
                LOG_WARN("KGS: {} frozen splats exceed max_cap {}; preserving frozen rows", frozen_count, cap);
                return;
            }
    ...
    lfs::core::Tensor KGS::get_active_indices() const {
        const size_t current_size = static_cast<size_t>(_splat_data->size());
        if (current_size == 0) {
            return {};
        }
    ...
    void KGS::mark_as_free(const lfs::core::Tensor& indices) {
    ...
    std::pair<lfs::core::Tensor, int64_t> KGS::fill_free_slots_with_data(
    ...
    void KGS::ensure_auxiliary_capacity_for_growth(const size_t target_size) {
        using namespace lfs::core;
        if (target_size == 0) {
            return;
        }
    ...
    void KGS::seed_from_view(int iter, const RenderOutput& render_output) {
        using namespace lfs::core;
        if (!background_improvements_enabled() ||
    ...
    void KGS::compute_bounds() {
        ...
        if (!finite_bounds || candidate.max_extent <= extent_epsilon) {
            if (!_bounds_valid) {
                LOG_WARN("KGS: spatial bounds unavailable for a degenerate active model; "
    ...
    void KGS::sync_mean_learning_rate() {
    ...
    void KGS::remove_gaussians(const lfs::core::Tensor& mask) {
        using namespace lfs::core;
        ...
        LOG_INFO("KGS::remove_gaussians: mask size={}, n_remove={}, current size={}",
    ...
    lfs::core::Tensor KGS::edge_guidance_factor() {
    ...
    void KGS::serialize(std::ostream& os) const {
    ...
    void KGS::deserialize(std::istream& is) {
    ...
    bool KGS::can_adopt_checkpoint_state(const IStrategy& loaded) const noexcept {
        const auto* source = dynamic_cast<const KGS*>(&loaded);
    ...
    void KGS::adopt_checkpoint_state(IStrategy& loaded) noexcept {
        auto& source = checked_checkpoint_source<KGS>(loaded);
        if (_optimizer)
    ...
    void KGS::reserve_optimizer_capacity(size_t capacity) {
        if (_optimizer) {
            _optimizer->reserve_capacity(capacity);
            LOG_INFO("KGS: reserved optimizer capacity for {} Gaussians", capacity);
        }
    }

    void KGS::set_optimization_params(const lfs::core::param::OptimizationParameters& params) {
    ...
    void KGS::refresh_decay_schedule_from_current_state() {
